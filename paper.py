...
class ArxivPaper:
    ...
    def is_valid(self) -> bool:
        ...
        if self._paper.pdf_url is None:
            logger.warning(f"Paper {self.arxiv_id} has no PDF URL, marking as invalid.")
            return False
        return True
    ...
    def tex(self) -> Optional[dict[str,str]]:
        ...
        try:
            with ExitStack() as stack:
                ...
                try:
                    ...
                    if e.code == 404:
                        ...
                        logger.debug(f"Source for {self.arxiv_id} not found (404). Skipping source analysis.")
                        return None
                    else:
                        # 如果是其他 HTTP 错误 (如 503)，这可能是临时性问题
                        logger.warning(f"HTTP Error {e.code} when downloading source for {self.arxiv_id}: {e.reason}")
                        return None
                except AttributeError as e:
                    # 捕获 AttributeError (pdf_url 为 None 导致的)
                    logger.warning(f"AttributeError when downloading source for {self.arxiv_id}: {e}. PDF URL might be None.")
                    return None
                except Exception as e:
                    # 捕获其他所有下载相关的异常
                    logger.warning(f"Unexpected error when downloading source for {self.arxiv_id}: {type(e).__name__}: {e}")
                    return None
                ...
                try:
                    ...
                    logger.debug(f"Failed to open tar file of {self.arxiv_id}: {type(e).__name__}: {e}")
                    return None
                ...
                for t in tex_files:
                    try:
                        f = tar.extractfile(t)
                        if f is None:
                            logger.debug(f"Failed to extract {t} from {self.arxiv_id}")
                            continue
                        ...
                        logger.debug(f"Failed to read tex file {t} of {self.arxiv_id}: {type(e).__name__}: {e}")
                        continue
                
                if main_tex is not None and main_tex in file_contents:
            ...
            # 最外层的异常捕获，确保任何未预料的错误都不会导致程序崩溃
            logger.error(f"Critical error when processing tex for {self.arxiv_id}: {type(e).__name__}: {e}")
            return None
    ...
    def tldr(self) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Error when generating TLDR for {self.arxiv_id}: {type(e).__name__}: {e}")
            return None
        ...
        try:
            if self.tex is not None:
                ...
                try:
                    ...
                    logger.debug(f"Failed to parse affiliations of {self.arxiv_id}: {e}")
                    return None
                return affiliations
            return None
        except Exception as e:
            logger.error(f"Error when extracting affiliations for {self.arxiv_id}: {type(e).__name__}: {e}")
            return None
